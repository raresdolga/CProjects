            ...
            while(*(g+i)!='\0')
                {
                    ...
                    i++;
            if(nr==x+1&&x!=0){
                if(strchr(">^v",*(g+i))!=0) bin=3;
                break;
               }
               else
               if(x==1)
                {
                    if(strchr(g,'>')!=0||strchr(g,'v')!=0||strchr(g,'^')!=0)bin=3;
...
int ok=0;
    ...
    if(*nm=='X')
    {
        i=*(nm+2)-'0'-72;
        j=*(nm+3)-'0'-72;
        if(a[V[i]][V[j]]==1)fml=1;
        else
            fml=0;
    }
    else
        if(*nm=='-')fml=(1-eval(nm+1,edges,size,V));
    else
        if(*nm=='(')
        {
            i=1;x=0;nr=0;
